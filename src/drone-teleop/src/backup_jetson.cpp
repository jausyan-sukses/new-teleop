...
public:
    TeleopDrone() : Node("teleop_drone")
    {
        ...
        pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/mavros/setpoint_position/local", 10);
        ...
        local_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
            "/mavros/local_position/pose", 10, std::bind(&TeleopDrone::poseCallback, this, std::placeholders::_1));
...
private:
    ...
    void poseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Current yaw=%.2f", current_yaw_);
    }
    ...
    void setModeGuided()
    {
        ...
        {
            RCLCPP_INFO(this->get_logger(), "Mode set to GUIDED successfully");
        }
    }
    ...
    void takeoffDrone()
    {
        ...
        takeoff_request->altitude = 1.5; // Takeoff ke 3 meter
            ...
            rclcpp::FutureReturnCode::SUCCESS)
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Drone took off successfully, %d", response->success);
        }
        else
        {
    ...
    void landDrone()
    {
        ...
        if (rclcpp::spin_until_future_complete(this->get_node_base_interface(), future) !=
            rclcpp::FutureReturnCode::SUCCESS)
        {
            RCLCPP_ERROR(this->get_logger(), "Failed to land drone");
        }
        else
        {
    ...
    void run()
    {
        ...
        while (rclcpp::ok())
        {
            ...
            {
            case 's':
                msg.linear.x = 0.5;  // Maju inverse
                break;
            case 'w':
                msg.linear.x = -0.5; // Mundur inverse
                break;
            case 'd':
            ...
            case 'e':
                ...
                break;
            case 'r':
                msg.linear.z = 0.2;  // Naik
                break;
            case 'f':
                msg.linear.z = -0.2; // Turun
                break;
            case 'm':
            ...
            // RCLCPP_INFO(this->get_logger(), "Sending command: x=%.2f, y=%.2f, z=%.2f, yaw=%.2f", msg.linear.x, msg.linear.y, msg.linear.z, msg.angular.z);
        }
    }
};
